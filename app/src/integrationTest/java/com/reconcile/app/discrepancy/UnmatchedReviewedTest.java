...
@Testcontainers
@TenantTest
@DisplayName("UnmatchedReviewed — accepting an UNMATCHED discrepancy as reviewed")
class UnmatchedReviewedTest {
    ...
    @Test
    @DisplayName(
            "POST .../resolve on an UNMATCHED discrepancy with empty body → 200, status=RESOLVED, kind=UNMATCHED_REVIEWED")
    void acceptUnmatchedAsReviewed(TenantId tenantId) throws Exception {
        ...
        jdbcTemplate.execute((ConnectionCallback<Void>) conn -> {
            ...
            try {
                ...
                try (ResultSet rs = conn.prepareStatement(
                                "SELECT kind FROM resolution WHERE discrepancy_id = '" + discrepancyId + "'")
                        .executeQuery()) {
                    assertThat(rs.next()).isTrue();
                    assertThat(rs.getString("kind")).isEqualTo("UNMATCHED_REVIEWED");
