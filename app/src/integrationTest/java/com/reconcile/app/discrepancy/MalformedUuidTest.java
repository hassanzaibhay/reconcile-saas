...
@Testcontainers
@TenantTest
@DisplayName("MalformedUuid — a structurally invalid UUID field is rejected with 400")
class MalformedUuidTest {
    ...
    @Test
    @DisplayName("pairing with a non-UUID string for 'a' → 400, not 500")
    void malformedUuidRejected(TenantId tenantId) throws Exception {
        ...
        mockMvc.perform(post("/api/v1/discrepancies/{id}/resolve", discrepancyId)
                        ...
                        .content("{\"pairings\":[{\"a\":\"not-a-uuid\",\"b\":\"" + UUID.randomUUID()
                                + "\"}],\"leftUnmatched\":[]}"))
