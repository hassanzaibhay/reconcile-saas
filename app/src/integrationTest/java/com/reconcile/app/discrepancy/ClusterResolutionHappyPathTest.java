...
@Testcontainers
@TenantTest
@DisplayName("ClusterResolutionHappyPath — 3-way cluster resolved with 1 pairing + 1 unmatched residual")
class ClusterResolutionHappyPathTest {
    ...
    @Test
    @DisplayName(
            "1 pairing + 1 residual → 200, match_result MANUAL_RESOLUTION row, matched_entry pair, discrepancy RESOLVED")
    void resolveClusterWithPairingAndResidual(TenantId tenantId) throws Exception {
        ...
        jdbcTemplate.execute((ConnectionCallback<Void>) conn -> {
            ...
            try {
                ...
                UUID matchResultId;
                try (ResultSet rs = conn.prepareStatement("SELECT id, left_entry_id, right_entry_id, rule_id"
                                + " FROM match_result WHERE rule_id = 'MANUAL_RESOLUTION'")
                        .executeQuery()) {
                    assertThat(rs.next()).isTrue();
                    matchResultId = (UUID) rs.getObject("id");
                    assertThat(List.of(rs.getObject("left_entry_id"), rs.getObject("right_entry_id")))
                ...
                try (ResultSet rs = conn.prepareStatement("SELECT ledger_entry_id FROM matched_entry"
                    ...
                    assertThat(ids).containsExactlyInAnyOrder(a.id().value(), b.id().value());
                }
    ...
    UUID queryDiscrepancyId(String schema, MatchRunId runId) {
